...
pub trait FetchCertificate {
    type ErrorCode = CertificateReadErr;
...
pub enum CertificateReadErr {
    FailGetCertificate,
}

impl ink_env::chain_extension::FromStatusCode for CertificateReadErr {
    fn from_status_code(status_code: u32) -> Result<(), Self> {
        match status_code {
            0 => Ok(()),
            1 => Err(Self::FailGetCertificate),
            _ => panic!("try to get certificate but encountered unknown status code"),
        }
    }
}
...
impl Environment for CustomEnvironment {
    ...
    type ChainExtension = FetchCertificate;
}
