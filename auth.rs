...
use ink_lang as ink;
use crate::blbc::{AuthCreated, Blbc, EVENT_ID_FOR_RETURNED_VALUE};
...
pub fn create_auth_request(ctx: &mut Blbc, auth_session_id: String, auth_request: AuthRequest, event_id: Option<String>) -> Result<(), String> {
    ...
    ctx.auth_request_map.insert(auth_session_id.clone(), &auth_request_stored);
    ctx.auth_session_ids.push(auth_session_id.clone());


    // 通过事件返回值
    ctx.env().emit_event(AuthCreated {
...
pub fn create_auth_response(ctx: &mut Blbc, auth_session_id: String, auth_response: AuthResponse, event_id: Option<String>) -> Result<(), String> {
    ink_env::debug_println!("---");
    ink_env::debug_println!("create_auth_response()");

    // 检查授权会话的请求是否存在
    let auth_request_stored = match ctx.auth_request_map.get(&auth_session_id) {
        None => { return Err(format!("授权会话 '{}' 不存在", auth_session_id)); }
        Some(r) => { r }
    };
    ...
    let metadata = match ctx.res_metadata_map.get(resource_id) {
        None => { return Err(format!("资源 '{}' 不存在", resource_id)); }
        Some(r) => { r }
    };
    ...
    // 存储数据
    ink_env::debug_println!("正在存储数据");
    ctx.auth_response_map.insert(auth_session_id.clone(), &auth_response_stored);
...
// 获取当前调用者尚未批复的请求
pub fn list_pending_auth_session_ids_by_resource_creator(ctx: &mut Blbc, page_size: u32, bookmark: Option<String>) -> Result<IDsWithPagination, String> {
    if page_size < 1 {
...
pub fn list_auth_session_ids_by_requestor(ctx: &mut Blbc, page_size: u32, bookmark: Option<String>, is_latest_first: bool) -> Result<IDsWithPagination, String> {
    if page_size < 1 {
        return Err("page_size 应为正整数".into());
    }
