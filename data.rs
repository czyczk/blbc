...
pub fn create_offchain_data(
    ...
    ctx.res_cid_map.insert(resource_id.clone(), &offchain_data.cid);
...
pub fn create_encrypted_data(
    ...
    // if size_stored != encrypted_data.metadata.size {
    //     return Err(format!(
    //         "大小不匹配，应有大小为 {}，实际大小为 {}",
    //         encrypted_data.metadata.size, size_stored
    //     ));
    // }

    let hash_stored = ctx.env().hash_bytes::<Sha2x256>(&data_bytes);
    let hash_stored_base64 = base64::encode(hash_stored);
    if hash_stored_base64 != encrypted_data.metadata.hash {
